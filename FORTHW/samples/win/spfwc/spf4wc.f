...
S" lru.f"	INCLUDED  \ aooa? history (last recently used)
...
: GETLINECOUNT ( -- n ) \ aica?auaao ?enei no?ie a eiiniee
  0 0 EM_GETLINECOUNT SendToCl
;
: LastLineIndex ( -- n)
  0  GETLINECOUNT 1-  EM_LINEINDEX SendToCl
;
: CurrentLine ( -- n) \ aica?auaao iiia? oaeouae no?iee
...
: GetClTextLength ( -- n) \ aica?auaao aeeio oaenoa a eiiniee
...
: LastLineChange ( addr -- )
  \ Caiaiyao iineaai?? no?ieo oaenoii - i?iiio+no?iea addr
...
;
: GetPrompt \ eeaaao niaa??eiia iineaaiae no?iee a aooa? - i?iiio
  \ Anee no?iea ionoay - auaaao noaiaa?oiue i?iiio
   C/L promptbuf W!
...
: VOC-NAME> ( wid -- s ) \ eiy nienea neia, anee ii eiaiiaai
...
;
\ iaiiaeyao ioia?a?aiea iai?, ooeaa?a e noaoonaa?a
: RefreshMenus { \ st curtls -- }
  \ auaia eioi?iaoee a noaoonaa?
  \ aa?ai aaiiua ec oeia aeaaiiai iioiea
  ViewSB IF
    TlsIndex@ TO curtls
...
: SaveScript { \ SizeEd Content fid -- }
  \ iieo?aai niaa??eiia ieia Script
  ...
  \ caienuaaai eia a oaee
  Content SizeEd 1- fid WRITE-FILE THROW
  \ cae?uaaai oaee e aooa?
  Content FREE THROW
...
;
\ ?enoao ia?aiauaaiue ai?eciioaeuiue nieeooa?
\ y - dragY, x - io 0 ai Myhwnd_width-1
...
VK_UP :M ( -- 0 FALSE | TRUE )
  VK_SHIFT GetKeyState 0x80 AND 0= IF \ anee ia ia?ao Shift
...
VK_RETURN :M
  CurrentLine DUP >R \ oaeou?? eeie?
  C/L tib W!
  tib SWAP EM_GETLINE SendToCl TO >in \ a tib
  \ anee iineaaiyy no?iea, oi oaaeei ec tib'a i?iiio
...
WM_LBUTTONDOWN :M ( lpar wpar msg hwnd -- 0 FALSE | TRUE )
  \ anee uaeeioee iuou? ia i?iiioa, oi iaai onoaiiaeou
  \ eo?ni? a eiiao i?iiioa
...
: WMLBUTTONDOWN { hwnd wy \ [ RECT::/SIZE ] rect [ POINT::/SIZE ] point }
  ...
  \ caaaai a?aieou, ca eioi?ua iuou ia ii?ao aueoe
...
WM_CAPTURECHANGED :M \ ieii iioa?yei oieon eee aue ReleaseCapture
  2DROP DROP
  DrawSplitter
  0 ClipCursor DROP
  0
...
WM_CREATE :M { lpar wpar hwnd \ hdc -- 0 }
  ...
  2DUP sViewTB     TRUE GetIniInt TO ViewTB
       sSplitRatio 100 GetIniInt 0 MAX 100 MIN TO SplitRatio
          \   ======75 ================
  ...
  THEN
\  =======================================================================
\  SBhwnd     clhwnd      edhwnd             TBhwnd (toolbar)
\  =======================================================================
...
\    Z" SP-FORTH 4.0 win console ver 0.6.13" CONS_ lpszClassName !
 Z" ****  ForthWin  Winconsole **** " CONS_ lpszClassName !
  }}
...
: CONSOLE
  0 -11 SetStdHandle DROP \ A WinXP GUI-i?eei?aiea i?e caionea ?a?ac eeie
  0 TO H-STDOUT           \ iieo?aao noaiaa?oiua oyiaeu
