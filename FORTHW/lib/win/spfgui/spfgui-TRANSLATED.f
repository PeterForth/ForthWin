...
Lib / win \ spfgui \ dtyps.f REQUIRE / MSG
...
: TYPE-GUI (addr u -)
    H-STDOUT 0>
    IF TYPE1 EXIT THEN \ If we are writing to a file for example ...
...
: CE-CON-MAIN
 ...
 \ fill structure
  / winclass TEST_window.size_struct!
  CS_HREDRAW CS_VREDRAW OR
  CS_OWNDC OR  
                              TEST_ window.style!
  ['] ConsoleWndProc TEST_window.procedure!
  0 TEST_ window.class +!
  0 TEST_window.window +!
  HINST TEST_window.example!
  1 HINST LoadIconA TEST_ windows.icon!
  IDC_ARROW 0 LoadCursorA TEST_ windows.cursor!
  BLACK_BRUSH GetStockObject TEST_ windows. Background!
  0 TEST_ window.menu!
\ S "SP-FORTH 4.0 GUI console ver. 0.5" 
  S "* FORTHWIN GRAPHICS CONSOLE 2019 *" DROP TEST_window.name!
  1 HINST LoadIconA TEST_ windows.icon +!

  TEST_ RegisterClassExA 0 = ABORT "#Class was not registered!"
  ...
  TEST_window.name @ DUP \ address of registered class name
